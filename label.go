...
func newResizableLabelRender(l *resizableLabel) *resizableLabelRender {
	r := &resizableLabelRender{
		label:            l,
		overallContainer: fyne.NewContainerWithoutLayout(),
		canvasText:       canvas.NewText(string(l.Get()), theme.TextColor()),
		mux:              new(sync.RWMutex),
	}
...
func (rlr *resizableLabelRender) Layout(layoutsize fyne.Size) {
	...
	rlr.overallContainer = fyne.NewContainerWithoutLayout()

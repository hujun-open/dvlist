...
func NewListHeaderRow(list []string, padding float32, handler headerOnTapHandler, dh notifyListDraghandler, arr []float32, look *Look) *listRow {
	...
	if len(arr) == 0 {
		...
		for _, p := range arr {
			row.arrangements = append(row.arrangements, p)
		}
	}
...
func (row *listRow) adjustArrangements(col int, delta float32) error {
	...
	if col < 0 || col >= len(row.arrangements)-1 {
		if col == len(row.arrangements)-1 {
	...
	if delta > 0 {
		row.arrangements[col]++
	} else {
		row.arrangements[col]--
		if row.arrangements[col] < 1 {
	...
	for i := col + 1; i < len(row.arrangements); i++ {
		row.arrangements[i] = (available * row.arrangements[i]) / previousRightTotal
		if row.arrangements[i] < 1 {
			row.arrangements[i] = 1
		}
	}
	// log.Printf("now arrangements is %v", row.arrangements)
	return nil
}
...
func (row *listRow) dragHandler(metadata interface{}, evt *fyne.DragEvent) {
	...
	if err == nil {
		...
		if row.headerDragHandler != nil {
			row.headerDragHandler(row.arrangements)
		}
	} else {
...
func newListRowRender(row *listRow) *listRowRender {
	...
	lrr.innerContainer = fyne.NewContainerWithoutLayout()
	...
	lrr.overallContainer = fyne.NewContainerWithLayout(layout.NewMaxLayout(),
		lrr.background, lrr.innerContainer)
	return lrr
}
